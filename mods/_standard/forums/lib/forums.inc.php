...
/****************************************************************************/
// $Id: forums.inc.php 10540 2010-12-16 19:35:58Z hwong $
...
* Adds a forum
* @access  public
* @param   array $course_data			add-forum form variables
* @see     $db					in include/vitals.inc.php
* @see     $addslashes			in include/vitals.inc.php
* @author  Heidi Hazelton
*/
function add_forum($course_data) {
	global $db;
	global $addslashes;

	$course_data['title'] = $addslashes($course_data['title']);
	$course_data['body']  = $addslashes($course_data['body']);
	$course_data['edit']  = intval($course_data['edit']);

	$sql	= "INSERT INTO ".TABLE_PREFIX."forums VALUES (NULL,'$course_data[title]', '$course_data[body]', 0, 0, NOW(), $course_data[edit])";
	$result = mysql_query($sql,$db);
...
* @author  Heidi Hazelton
*/
function edit_forum($course_data) {
	global $db;
	global $addslashes;
	...
	$course_data['fid']    = intval($course_data['fid']);
	$course_data['edit']    = intval($course_data['edit']);

	$sql	= "UPDATE ".TABLE_PREFIX."forums SET title='$course_data[title]', description='$course_data[body]', last_post=last_post, mins_to_edit=$course_data[edit] WHERE forum_id=$course_data[fid]";
	$result = mysql_query($sql,$db);
