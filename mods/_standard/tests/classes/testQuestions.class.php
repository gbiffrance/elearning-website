...
/****************************************************************/
// $Id: testQuestions.class.php 10339 2010-10-19 18:43:17Z cindy $
...
class OrderingQuestion extends AbstractTestQuestion {
	...
	/*public */function mark($row) { 
		$num_choices = count($course_data['answers'][$row['question_id']]);
		...
		for ($i = 0; $i < $num_choices ; $i++) {
			$course_data['answers'][$row['question_id']][$i] = intval($course_data['answers'][$row['question_id']][$i]);

			if ($course_data['answers'][$row['question_id']][$i] == -1) {
				// nothing to do. it was left blank
			} else if ($course_data['answers'][$row['question_id']][$i] == $answers[$i]) {
				$num_answer_correct++;
			}
			$ordered_answers[$answers[$i]] = $course_data['answers'][$row['question_id']][$i];
		}
		...
		$course_data['answers'][$row['question_id']] = implode('|', $ordered_answers);

		return $score;
	}

	//QTI Import Ordering Question
	function importQTI($course_data){
		global $msg, $db;

		if ($course_data['question'] == ''){
			$missing_fields[] = _AT('question');
		}

		if (trim($course_data['choice'][0]) == '') {
			$missing_fields[] = _AT('item').' 1';
		}
		if (trim($course_data['choice'][1]) == '') {
		...
		if (!$msg->containsErrors()) {
			...
			for ($i=0; $i<10; $i++) {
				 ...
				 * @harris
				 */
				$course_data['choice'][$i] = validate_length($course_data['choice'][$i], 255);
				$course_data['choice'][$i] = trim($course_data['choice'][$i]);

				if ($course_data['choice'][$i] != '') {
					/* filter out empty choices/ remove gaps */
					$choice_new[] = $course_data['choice'][$i];
					$answer_new[] = $order++;
				}
			}

			$course_data['choice']   = array_pad($choice_new, 10, '');
			$answer_new        = array_pad($answer_new, 10, 0);
//			$course_data['feedback'] = $addslashes($course_data['feedback']);
//			$course_data['question'] = $addslashes($course_data['question']);
		
			$sql_params = array(	$course_data['category_id'], 
									$_SESSION['course_id'],
									$course_data['feedback'], 
									$course_data['question'], 
									$course_data['choice'][0], 
									$course_data['choice'][1], 
									$course_data['choice'][2], 
									$course_data['choice'][3], 
									$course_data['choice'][4], 
									$course_data['choice'][5], 
									$course_data['choice'][6], 
									$course_data['choice'][7], 
									$course_data['choice'][8], 
									$course_data['choice'][9], 
									$answer_new[0], 
									$answer_new[1], 
									$answer_new[2], 
									$answer_new[3], 
...
class TruefalseQuestion extends AbstracttestQuestion {
	...
	/*public */function mark($row) { 
		$course_data['answers'][$row['question_id']] = intval($course_data['answers'][$row['question_id']]);

		if ($row['answer_0'] == $course_data['answers'][$row['question_id']]) {
	...
	function importQTI($course_data){
		...
		if  ($course_data['answer'] == 'ChoiceT'){
			$course_data['answer'] = 1;
		} else {
			$course_data['answer'] = 2;	
		}
...
class LikertQuestion extends AbstracttestQuestion {
	...
	function importQTI($course_data){
		global $msg, $db;
//		$course_data = $this->_POST; 

		$empty_fields = array();
		if ($course_data['question'] == ''){
			$empty_fields[] = _AT('question');
		}
		if ($course_data['choice'][0] == '') {
			$empty_fields[] = _AT('choice').' 1';
		}

		if ($course_data['choice'][1] == '') {
...
//			$course_data['question']   = $addslashes($course_data['question']);
			...
			for ($i=0; $i<10; $i++) {
				$course_data['choice'][$i] = trim($course_data['choice'][$i]);
				$course_data['answer'][$i] = intval($course_data['answer'][$i]);

				if ($course_data['choice'][$i] == '') {
					/* an empty option can't be correct */
					$course_data['answer'][$i] = 0;
				}
			}
			...
			$sql_params = array(	$course_data['category_id'], 
									...
									$course_data['choice'][8], 
									$course_data['choice'][9], 
									$course_data['answer'][0], 
									$course_data['answer'][1], 
									$course_data['answer'][2], 
									$course_data['answer'][3], 
									$course_data['answer'][4], 
									$course_data['answer'][5], 
									$course_data['answer'][6], 
									$course_data['answer'][7], 
									$course_data['answer'][8], 
									$course_data['answer'][9]);
...
class LongQuestion extends AbstracttestQuestion {
	...
	/*public */function mark($row) { 
		global $addslashes;
		$course_data['answers'][$row['question_id']] = $addslashes($course_data['answers'][$row['question_id']]);
		return NULL;
	}
...
//			$course_data['question'] = $addslashes($course_data['question']);
			...
			if ($course_data['property']==''){
				$course_data['property'] = 4;	//essay
			}
...
class MatchingQuestion extends AbstracttestQuestion {
	...
	/*public */function mark($row) { 
		$num_choices = count($course_data['answers'][$row['question_id']]);
		$num_answer_correct = 0;
		foreach ($course_data['answers'][$row['question_id']] as $item_id => $response) {
			if ($row['answer_' . $item_id] == $response) {
				$num_answer_correct++;
			}
			$course_data['answers'][$row['question_id']][$item_id] = intval($course_data['answers'][$row['question_id']][$item_id]);
		}
		...
		$course_data['answers'][$row['question_id']] = implode('|', $course_data['answers'][$row['question_id']]);
...
//		$course_data = $this->_POST; 
		...
		if (!is_array($course_data['answer'])){
			$temp = $course_data['answer'];
			$course_data['answer'] = array();
			$course_data['answer'][0] = $temp;
		}
		ksort($course_data['answer']);	//array_pad returns an array disregard of the array keys
		//default for matching is '-'
		$course_data['answer']= array_pad($course_data['answer'], 10, -1);

		for ($i = 0 ; $i < 10; $i++) {
			$course_data['groups'][$i]        = trim($course_data['groups'][$i]);
			$course_data['answer'][$i] = (int) $course_data['answer'][$i];
			$course_data['choice'][$i]          = trim($course_data['choice'][$i]);
		}

		if (!$course_data['groups'][0] 
			|| !$course_data['groups'][1] 
			|| !$course_data['choice'][0] 
			|| !$course_data['choice'][1]) {
//			$msg->addError('QUESTION_EMPTY');
		...
		if (!$msg->containsErrors()) {
//			$course_data['feedback']     = $addslashes($course_data['feedback']);
//			$course_data['instructions'] = $addslashes($course_data['instructions']);
		
			$sql_params = array(	$course_data['category_id'], 
									$_SESSION['course_id'],
									$course_data['feedback'], 
									$course_data['question'], 
									$course_data['groups'][0], 
									$course_data['groups'][1], 
									$course_data['groups'][2], 
									$course_data['groups'][3], 
									$course_data['groups'][4], 
									$course_data['groups'][5], 
									$course_data['groups'][6], 
									$course_data['groups'][7], 
									$course_data['groups'][8], 
									$course_data['groups'][9], 
									$course_data['answer'][0], 
									$course_data['answer'][1], 
									$course_data['answer'][2], 
									$course_data['answer'][3], 
...
class MultichoiceQuestion extends AbstracttestQuestion {
	...
	/*public */function mark($row) { 
		$score = 0;
		$course_data['answers'][$row['question_id']] = intval($course_data['answers'][$row['question_id']]);
		if ($row['answer_' . $course_data['answers'][$row['question_id']]]) {
			$score = $row['weight'];
		} else if ($course_data['answers'][$row['question_id']] == -1) {
			$has_answer = 0;
			for($i=0; $i<10; $i++) {
...
class MultianswerQuestion extends MultichoiceQuestion {
	...
	/*public */function mark($row) { 
		...
		if (is_array($course_data['answers'][$row['question_id']]) && count($course_data['answers'][$row['question_id']]) > 1) {
			if (($i = array_search('-1', $course_data['answers'][$row['question_id']])) !== FALSE) {
				unset($course_data['answers'][$row['question_id']][$i]);
			}
			$num_answer_correct = 0;
			foreach ($course_data['answers'][$row['question_id']] as $item_id => $answer) {
			...
			// no answer given
			$course_data['answers'][$row['question_id']] = '-1'; // left blank
			$score = 0;
		}
		return $score;
	}
...
//		$course_data = $this->_POST; 
		...
		//The following code will change $course_data[answer] back to a vector.
		$course_data['answer'] = $course_data['answers'];

		if (!$msg->containsErrors()) {
			$choice_new = array(); // stores the non-blank choices
			$answer_new = array(); // stores the associated "answer" for the choices

			foreach ($course_data['choice'] as $choiceNum=>$choiceOpt) {
				$choiceOpt = validate_length($choiceOpt, 255);
				$choiceOpt = trim($choiceOpt);
				$course_data['answer'][$choiceNum] = intval($course_data['answer'][$choiceNum]);
				if ($choiceOpt == '') {
					/* an empty option can't be correct */
					$course_data['answer'][$choiceNum] = 0;
				} else {
					/* filter out empty choices/ remove gaps */
					$choice_new[] = $choiceOpt;
					if (in_array($choiceNum, $course_data['answer'])){
						$answer_new[] = 1;
					} else {
						$answer_new[] = 0;
					}

					if ($course_data['answer'][$choiceNum] != 0)
						$has_answer = TRUE;
				}
			}
			...
			if ($has_answer != TRUE) {
				...
				$hidden_vars['required']    = htmlspecialchars($course_data['required']);
				$hidden_vars['feedback']    = htmlspecialchars($course_data['feedback']);
				$hidden_vars['question']    = htmlspecialchars($course_data['question']);
				$hidden_vars['category_id'] = htmlspecialchars($course_data['category_id']);
				...
				$course_data['answer'] = $answer_new;
				$course_data['choice'] = $choice_new;
				$course_data['answer'] = array_pad($course_data['answer'], 10, 0);
				$course_data['choice'] = array_pad($course_data['choice'], 10, '');
